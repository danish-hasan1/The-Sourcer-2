...
async def run_sourcing(
    ...
    # Fallback: if no location in top-level, try to extract from role_objective or ideal_candidate_brief
    if not location:
        role_obj = analysis.get("role_objective", "")
        # Common location patterns injected by the LLM into role_objective
        import re
        loc_match = re.search(r'\b([A-Z][a-z]+(?:,\s*[A-Z][a-z]+)*)\b', role_obj)
        if loc_match:
    ...
    if not boolean or len(boolean) < 10:
        ...
        role_obj = analysis.get("role_objective", "")
        if titles:
            title_query = " OR ".join(f'"{t}"' for t in titles[:3])
            boolean = title_query
        elif role_obj:
            boolean = f'"{role_obj[:60]}"'
